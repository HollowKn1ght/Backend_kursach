...
class Action
{
	...
	function save_user()
	{
		...
		foreach ($_POST as $k => $v) {
			if (!in_array($k, array('id', 'cpass'))) { //Everything but the confirmation password
				if (!empty($data))
					$data .= ", ";
		...
		if ($_FILES['img']['tmp_name'] != '') {
			...
			$data .= ", avatar = '1604716320_no-image-available.png'";
		}
		if (empty($id)) {
	...
	function save_upload()
	{
		...
		if (empty($id)) {
			$data .= ", user_id = {$_SESSION['login_id']}, total_views = 0 ";
	...
	{
		extract($_POST);
		$save = $this->db->query("INSERT INTO likes set upload_id = '" . $upload_id . "', user_id = '" . $user_id . "', ip_address = '" . $ip_address . "'");
		return 1;
	}
	...
	{
		extract($_POST);
		$delete = $this->db->query("DELETE FROM likes where user_id = '" . $user_id . "'");
		return 1;
	}
}