...
if (isset($_GET['id'])) {
	$qry = DataBase::connToDB()->query("SELECT * FROM users where id = {$_GET['id']}");
...
<div class="container py-3">
	<div class="col-lg-12">
		<div class="card bg-light">
			<div class="card-body">
				<h3><b>
						<?php echo isset($id) ? "Manage" : 'Create' ?> Account
					</b></h3>
				<hr>
					...
					<div class="col-md-12">
						...
						<div class="col-lg-12 text-right justify-content-center d-flex">
							<button class="btn btn-primary mr-2">
								<?php echo isset($id) ? "Update" : 'Create' ?> Account
							</button>
							<button class="btn btn-secondary" type="reset">Clear</button>
				</form>
			</div>
			</div>
				<div id="divForbtnDel"></div>
			</div>
		</div>
	</div>
</div>
...
<script>
	...
	$('#manage_account').submit(function (e) {
		...
		$.ajax({
			url: '../../app/Controllers/ajax.php?action=save_user',
	...
	$(document).ready(function () {
		...
		var codeBlock = '<hr>' +
			'<div class="col-lg-12 text-right justify-content-center d-flex">' +
			'<button onclick="btnDel_func()" class="btn btn-danger mr-2">Delete Account</button>' +
			'</div>';
		document.getElementById("divForbtnDel").innerHTML = codeBlock;
		<?php endif; ?>
	});

	function btnDel_func(x){
		<?php if (isset($_SESSION['login_id'])): ?>
			let text = "Are you sure to delete account?";
			if (confirm(text) == false) {
				exit;
			} else{
				start_load();
				$.ajax({
					url: '../../app/Controllers/ajax.php?action=delete_user',
